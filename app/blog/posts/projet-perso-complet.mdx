...
J'ai décidé d'utiliser Angular pour le frontend de l'application qui me permet de créer une application web dynamique et réactive (grâce à son architecture basée sur les composants).
Il se base sur TypeScript, ce qui ajoute des fonctionnalités de typage statique à JavaScript, améliorant ainsi la qualité du code et facilitant la maintenance.

L'interface utilisateur permet aux utilisateurs de créer, afficher, mettre à jour et supprimer des listes et des tâches. De plus, les utilisateurs peuvent marquer les tâches comme terminées et archiver les listes.

Angular est un framework que je voulais apprendre depuis un moment, donc c'est une bonne opportunité pour moi de le faire.

En plus de l'outil principal Angular, j'utilise également Tailwind CSS pour le style de l'application. Tailwind est un framework CSS utilitaire qui permet de créer des interfaces utilisateur rapidement et efficacement en utilisant des classes prédéfinies.

Enfin, il y a PrimeNG qui est une bibliothèque de composants UI pour Angular. Elle offre une variété de composants prêts à l'emploi, tels que des boutons, des tableaux, des formulaires, etc., ce qui facilite le développement de l'interface utilisateur.
...
Jenkins est une plateforme permettant l'execution de pipelines permettant d'automatiser la vérification et le bon fonctionnement du projet dès lors qu'un changement est prêt à être inclus dans Git. Cela concerne donc les builds, tests maia aussi le déploiement, par exemple pour ce projet ça me permet actuellement de :
- Construire le backend et le frontend, et lancer leurs tests unitaires.
- Construire les différentes images docker (backend + frontend mais également les customisations de Prometheus et Spark décrites plus haut), et les mettre sur le registre Docker privé de mon serveur.
- Lancer des tests d'intégration.
- Déployer le projet en format production sur Kubernetes, dont un cluster avec un noeud qui est disponible sur mon serveur.

Tout comme le reste du projet, c'est un aspect qui évoluera au fil du temps.

### Les tests

Dans un but de garantir une non-regression et de valider le bon fonctionnement de l'application, plusieurs tests sont mis en place.
Des tests unitaires sont mis en place pour le backend avec JUnit et Mockito, couvrant les contrôlleurs pour l'instant sur les opérations décrites dans l'API.
Pour le frontend, des tests minimals des composants sont ajoutés avec Jest, cela permet de vérifier que les composants Angular sont bien créés.

D'autres types de tests sont à venir comme des tests d'intégration qui pourront idéalement être joués durant le CI/CD
